...
static cl_mem face_bbox_buf = NULL;
static cl_mem hole_radii_buf = NULL;
static cl_mem bbox_mask_buf = NULL;
static cl_mem inside_mask_buf = NULL;
...
static void cleanup() {
    ...
    if (face_bbox_buf) clReleaseMemObject(face_bbox_buf);
    if (hole_radii_buf) clReleaseMemObject(hole_radii_buf);
    if (bbox_mask_buf) clReleaseMemObject(bbox_mask_buf);
    if (inside_mask_buf) clReleaseMemObject(inside_mask_buf);
...
int main(int argc, char** argv) {
    ...
    int debug_counters = 0;
    int debug_disable_bbox = 0;
    ...
    for (size_t i = 0; i < extras.size(); ++i) {
        ...
        if (e == "-help" || e == "--help") {
            ...
            printf("  --quality <name>     hq | fast (default hq)\n");
            printf("  --debug-counters <0|1> enable render sanity counters\n");
            printf("  --debug-disable-bbox <0|1> bypass bbox culling (debug)\n");
            return 0;
        } else if (e == "--alpha") {
            ...
            std::string v; if (!next(v)) { fprintf(stderr, "invalid --quality\n"); return -1; }
            quality = v;
        } else if (e == "--debug-counters") {
            std::string v; if (!next(v) || !parse_int(v, debug_counters)) { fprintf(stderr, "invalid --debug-counters\n"); return -1; }
        } else if (e == "--debug-disable-bbox") {
            std::string v; if (!next(v) || !parse_int(v, debug_disable_bbox)) { fprintf(stderr, "invalid --debug-disable-bbox\n"); return -1; }
        }
    }

    if (quality != "hq" && quality != "fast") {
        fprintf(stderr, "invalid --quality (use hq or fast)\n");
        return -1;
    }
    bool is_hq = (quality == "hq");
    ...
    if (!is_hq) {
        if (!set_rings) rings = 0;
        if (!set_wire) wire = 0;
        if (!set_shadow) shadow = 0;
    }
    ...
    printf("Quality: %s (rings=%d wire=%d shadow=%d)\n", quality.c_str(), rings, wire, shadow);
    if (debug_counters) {
        printf("Debug counters: enabled\n");
    }
    if (debug_disable_bbox) {
        printf("Debug toggles: disable_bbox=%d\n", debug_disable_bbox);
    }
    ...
    std::vector<cl_float4> face_bbox(12);
    for (int f = 0; f < 12; ++f) {
        float minx2 = vbuf[faces[f][0]].s[0];
        float maxx2 = minx2;
        float miny2 = vbuf[faces[f][0]].s[1];
        float maxy2 = miny2;
        for (int k = 0; k < 5; ++k) {
            int ia = faces[f][k];
            float ax = vbuf[ia].s[0];
            float ay = vbuf[ia].s[1];
            minx2 = fminf(minx2, ax); maxx2 = fmaxf(maxx2, ax);
            miny2 = fminf(miny2, ay); maxy2 = fmaxf(maxy2, ay);
        }
        float pad = 1.5f;
        float bx0 = fmaxf(0.0f, minx2 - pad);
        float by0 = fmaxf(0.0f, miny2 - pad);
        float bx1 = fminf((float)(width - 1), maxx2 + pad);
        float by1 = fminf((float)(height - 1), maxy2 + pad);
        face_bbox[f] = {bx0, by0, bx1, by1};
    }
    ...
    for (int f = 0; f < 12; ++f) {
        ...
        Vec3 refl = reflect_vec({-light1.x, -light1.y, -light1.z}, n);
        float spec_pow = is_hq ? 60.0f : 30.0f;
        float spec = powf(fmaxf(0.0f, dot(refl, view_dir)), spec_pow);
        float ambient = is_hq ? 0.25f : 0.28f;
        float kd = is_hq ? 0.75f : 0.70f;
        float ks = is_hq ? 0.28f : 0.10f;
    ...
    face_bbox_buf = CL_CHECK2(clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                             sizeof(cl_float4) * face_bbox.size(), face_bbox.data(), &_err));
    ...
    size_t mask_bytes = debug_counters ? (static_cast<size_t>(width) * height) : 1;
    std::vector<uint8_t> mask_init(mask_bytes, 0);
    bbox_mask_buf = CL_CHECK2(clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                                             mask_bytes, mask_init.data(), &_err));
    inside_mask_buf = CL_CHECK2(clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                                               mask_bytes, mask_init.data(), &_err));
    ...
    CL_CHECK(clSetKernelArg(kernel, 8, sizeof(cl_mem), &face_bbox_buf));
    CL_CHECK(clSetKernelArg(kernel, 9, sizeof(cl_mem), &hole_radii_buf));
    CL_CHECK(clSetKernelArg(kernel, 10, sizeof(int), &width));
    CL_CHECK(clSetKernelArg(kernel, 11, sizeof(int), &height));
    CL_CHECK(clSetKernelArg(kernel, 12, sizeof(int), &holes));
    CL_CHECK(clSetKernelArg(kernel, 13, sizeof(int), &rings));
    CL_CHECK(clSetKernelArg(kernel, 14, sizeof(int), &wire));
    CL_CHECK(clSetKernelArg(kernel, 15, sizeof(float), &wire_thickness));

    cl_float2 cam_center = {cx, cy};
    float inv_scale = (scale != 0.0f) ? (1.0f / scale) : 1.0f;
    int quality_flag = is_hq ? 1 : 0;
    CL_CHECK(clSetKernelArg(kernel, 16, sizeof(int), &quality_flag));
    CL_CHECK(clSetKernelArg(kernel, 17, sizeof(int), &debug_disable_bbox));
    CL_CHECK(clSetKernelArg(kernel, 18, sizeof(cl_float2), &cam_center));
    CL_CHECK(clSetKernelArg(kernel, 19, sizeof(float), &inv_scale));
    CL_CHECK(clSetKernelArg(kernel, 20, sizeof(int), &shadow));
    ...
    CL_CHECK(clSetKernelArg(kernel, 21, sizeof(cl_float2), &shadow_center));
    CL_CHECK(clSetKernelArg(kernel, 22, sizeof(cl_float2), &shadow_radii));
    CL_CHECK(clSetKernelArg(kernel, 23, sizeof(float), &shadow_strength));
    CL_CHECK(clSetKernelArg(kernel, 24, sizeof(int), &debug_counters));
    CL_CHECK(clSetKernelArg(kernel, 25, sizeof(cl_mem), &bbox_mask_buf));
    CL_CHECK(clSetKernelArg(kernel, 26, sizeof(cl_mem), &inside_mask_buf));
    CL_CHECK(clSetKernelArg(kernel, 27, sizeof(cl_mem), &out_buf));
    ...
    if (debug_counters) {
        std::vector<uint8_t> bbox_mask(mask_bytes, 0);
        std::vector<uint8_t> inside_mask(mask_bytes, 0);
        CL_CHECK(clEnqueueReadBuffer(commandQueue, bbox_mask_buf, CL_TRUE, 0, mask_bytes, bbox_mask.data(), 0, NULL, NULL));
        CL_CHECK(clEnqueueReadBuffer(commandQueue, inside_mask_buf, CL_TRUE, 0, mask_bytes, inside_mask.data(), 0, NULL, NULL));
        uint32_t bbox_count = 0;
        uint32_t inside_count = 0;
        for (size_t i = 0; i < mask_bytes; ++i) {
            bbox_count += (bbox_mask[i] != 0);
            inside_count += (inside_mask[i] != 0);
        }
        printf("Counters: bbox_pixels=%u inside_pixels=%u\n", bbox_count, inside_count);
        if (inside_count < 1000) {
            fprintf(stderr, "Render sanity failed: inside_pixels too low (%u)\n", inside_count);
            cleanup();
            return -1;
        }
    }
